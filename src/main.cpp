...
void printHelpMessage() {
    ...
    paramsList << "\033[1;33m   mem\033[0;36m -> Turns on tracing of the specified memory range. e.g: 'mem 0x7f5310:0x7f531e', 'mem rsp+4:rsp+12'" << endl;
...
bool analyseParam(string param, string val) {
    if(!param.find("--")) {
        ...
        if(!paramName.compare("entry")) {
            ...
            int sep = val.find(":");
            if(sep != string::npos && sep != val.length()-1) {
                string val1 = val.substr(0,sep);
                string val2 = val.substr(sep+1);
                sep = val1.find("+");
